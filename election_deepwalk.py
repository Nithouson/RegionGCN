...
from reggcn import standardized_adj
...
model = Node2Vec(data.edge_index, embedding_dim=14, walk_length=20, context_size=10,
...
optimizer = torch.optim.SparseAdam(list(model.parameters()), lr=0.01)  # default: 0.01
...
def plot_points(epoch):
    model.eval()
    z = model().cpu().numpy()
    z = TSNE(n_components=2).fit_transform(z)
    y = data.y
    states = sorted(list(set(y)))
    fig = plt.figure(figsize=(12, 12))
    ax = fig.add_subplot(111)
    for i in range(len(states)):
        state = states[i]
        ax.scatter(z[y == state, 0], z[y == state, 1], s=8, label=state)
    plt.axis('off')
...
with torch.no_grad():
    z = model().cpu().numpy()
    emb_file = open("uselec_emb_.pkl", "wb")
